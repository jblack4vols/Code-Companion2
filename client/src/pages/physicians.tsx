...
export default function PhysiciansPage() {
  ...
  const toggleFavorite = useMutation({ mutationFn: async (id: string) => { if (favoriteIds.includes(id)) await apiRequest("DELETE", `/api/favorites/${id}`); else await apiRequest("POST", `/api/favorites/${id}`); }, onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/favorites"] }), onError: onErr });
