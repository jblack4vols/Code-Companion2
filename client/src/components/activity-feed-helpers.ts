...
export function getActivityDescription(activity: ActivityItem): string {
  switch (activity.activity_type) {
    case "interaction":
      return `${activity.user_name} logged a ${activity.type?.toLowerCase() ?? "contact"} with Dr. ${activity.physician_last_name ?? "Unknown"}`;
    case "referral":
      return `New referral from ${activity.physician_name ?? "Unknown"} at ${activity.location_name ?? ""}`.trim();
    case "task":
      return `${activity.user_name} completed: ${activity.summary ?? "a task"}`;
    case "provider":
      return `${activity.user_name} added a new provider`;
    default:
