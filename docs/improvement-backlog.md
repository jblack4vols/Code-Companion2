...
| 7 | P1 | **Referral-to-arrival conversion tracking per location** — surface `arrivedVisits / scheduledVisits` ratio on location dashboard card | 8 locations need to compare intake efficiency; data exists but no per-location breakdown in UI | S | `referrals.scheduledVisits`, `arrivedVisits` columns exist; `location_monthly_summary` has `referralsCount` but not arrival rate | PARTIAL — per-location metrics (`totalScheduled`, `totalArrived`, `arrivalRate`) added to conversion endpoint; UI integration pending |
