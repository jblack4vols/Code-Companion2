...
- **Workflow Tools**: Includes a calendar for event scheduling (with completion status toggling — completed events turn green), a task management system with email notifications, quick-add interaction logging, and a Daily Hit List (`/hit-list`) aggregating calendar events, tasks due, and follow-up interactions by day for the current week.
- **Interaction Management**: Interactions support soft delete and restore (reopen). Users can toggle "Show Deleted" to view and restore previously removed interactions.
